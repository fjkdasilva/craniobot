...
CHANGES:
Added abiity to load arbitrarily shaped craniotomies (not just circlar)
Added ability to do multiple craniotomies in one pass
Integrated with CranioDesigner, a craniotomy design package, developed by Flavio JK da Silva (4/2/2020)
"""
import pygame
...
BREGMAscrn = [217,440,580] # in pixels (BREGMA)
...
graphic_scaleXY = 10.0  #66 pix for 9mm dist from bregma to lambda = 7.333 pix/mm
graphic_scaleZ = 5.0
Robo000scrn = [0,0,0]
...
# screw holes
hole_idx = 0
screw_holes_rat = [(-2.67, -5.625), (2.67,-5.625), (5, 7.68), (2.65, 9), (-2.65, 9), (-5, 7.68), (-1,12) ]
screw_holes_scrn = []
...
craniotomies_rat = []
###########################################################################################################
# Craniotomy trajectory is a circle of r radius, drawn on best fit plane generated by 5 j=chosen ooints on the skull
###########################################################################################################
...
def CranioBot_GUI_ELEMENTS(myscreen):
    global drill_vel, hole_idx, craniotomies_rat, screw_holes_rat
    # BOXES
    boxes = []
    ...
    #                text_color = (0,0,0)):
    buttons.append(MyButton(myscreen,0, 500,20, 100,30,"HOME ROBOT",14))
    buttons.append(MyButton(myscreen,0, 500,60, 100,30,"TO BREGMA",14))
    
    buttons.append(MyButton(myscreen,1, 350,215, 120,30,"SELECT BREGMA",14))
##    buttons.append(MyButton(myscreen,11, 350,215, 120,30,"CLR POINT",14))
##    buttons.append(MyButton(myscreen,30, 350,255, 120,30,"BEST FIT PLANE",14))

    buttons.append(MyButton(myscreen,40, 60,290, 150,25,"Select Existing Craniotomy",14))
    buttons.append(MyButton(myscreen,41, 220,290, 150,25,"Create New Craniotomy Path",14))
...
##    labels.append(MyLabel(myscreen,50,20,   "1.) Align robot to steriotaxic equipment, then press 'HOME ROBOT'",16))
##    labels.append(MyLabel(myscreen,50,40,   "2.) Use screen buttons to move drill to bregma. ",16))
##    labels.append(MyLabel(myscreen,50,60,   "3.) Then press 'SELECT BREGMA'",16))
##    labels.append(MyLabel(myscreen,50,80,   "4.) Now use GUI to move Robot to set at least 4 more",16))
##    labels.append(MyLabel(myscreen,50,100,  "     points on skull. Press 'SELECT POINT'",16))
##    labels.append(MyLabel(myscreen,50,120,  "5.) Select best-fit plane through 5 points.",16))
    labels.append(MyLabel(myscreen,50,20,   "1.) Align robot to steriotaxic equipment, then press 'HOME ROBOT'",16))
    labels.append(MyLabel(myscreen,50,40,   "2.) Select species (rats or mice). ",16))
    labels.append(MyLabel(myscreen,50,60,   "3.) Use screen buttons to move drill to bregma. ",16))
    labels.append(MyLabel(myscreen,50,80,   "4.) Then press 'SELECT BREGMA'",16))
    labels.append(MyLabel(myscreen,100,200, "           Robo                      Rat",16))
    ...
    labels.append(MyLabel(myscreen,410,530, "X",14))
    labels.append(MyLabel(myscreen,468,480, "Y",14))
    labels.append(MyLabel(myscreen,590,480, "Z",14))
    labels.append(MyLabel(myscreen,630,585, "Skull Surface",14))
    labels.append(MyLabel(myscreen,640,480, "Home Z",14)) # Z axis
    labels.append(MyLabel(myscreen,345,330, "Home XY",14)) # XY axis

    labels.append(MyLabel(myscreen,50,125,   "Rats",16))
    labels.append(MyLabel(myscreen,50,150,   "Mice",16))
    
    # RADIO BUTTONS
    radio_buttons = []
    #(self,screen,index, x,y,radius, ONOFF, on_color)
    radio_buttons.append(radio_button(myscreen,0,   110,130, 7, "ON", (255,200,200))) # RATS
    radio_buttons.append(radio_button(myscreen,1,   110,155, 7, "OFF", (255,200,200))) # Mice

    """AtlasGuide: Software for stereotaxic guidance using 3D CT/MRI hybrid atlases of developing mouse brains
    Xin Li,a Manisha Aggarwal,a Johnny Hsu,a Hangyi Jiang,a and Susumu Moria,b,*
    J Neurosci Methods. Author manuscript; available in PMC 2014 Oct 30.

    bregma-lambda distance on mouse ~= 4.2 mm
    ...
    # ROBO COORDS
    # Point 1
    info_boxes.append(InfoBox( myscreen,100,220,120,20,"Bregma",'LEFT','0, 0, 0'))
    # Point 2
...
##    # Point 5
##    info_boxes.append(InfoBox( myscreen,100,260,120,20,"Other",'LEFT','0, 0, 0'))
...
##    # Point 5
##    info_boxes.append(InfoBox( myscreen,220,260,120,20,"",'LEFT','0, 0, 0'))
...
##    user_inputs.append(get_user_input( myscreen,373,340,50,17,"7.) Enter radius(remember to subtract drill diam)(mm)     ",'LEFT','5.0'))
    ...
    return buttons, radio_buttons, boxes, LEDs, toggles, info_boxes, user_inputs, labels
...
def draw_circle_craniotomy(myscreen,user_inputs): # User inputs are offset from bregma and Raduis in mm
...
def setupGUI():
    ...
    moveThreadZ = None
    # GLOBALS
    global BREGMAscrn, BREGMArobo, graphic_scaleXY, graphic_scaleZ
    global screw_holes_rat, screw_holes_scrn, craniotomies, craniotomies_rat, craniotomies_scrn,drill_vel
    ...
    rats_skull_graphic = pygame.image.load(r'.\RESOURCES\skullgraphicReal.png')
    mice_skull_graphic = pygame.image.load(r'.\RESOURCES\mouse.png')
    pygame.display.set_caption('CraniobotGUI.py 2.3 by F. da Silva & Mark Schatza Nov. 20, 2019 (updated 4/6/2020)') # Enter your window caption here
    ...
    #  Create GUI elements
    #########################
    buttons, radio_buttons, boxes,LEDs,toggles,info_boxes,user_inputs,labels = CranioBot_GUI_ELEMENTS( myscreen )
    drill = MyDrill(myscreen,215, 473)
    drill_static_graphic = MyDrill(myscreen,510, 540)
    # WARNING MSGS
    warning = WARNINGBox( myscreen,500,150,160,160,"",'LEFT','WARNING!!',22)
            ...
            # MOUSE DOWN
                ...
                if LEFT_MOUSE_DOWN:
                    ...
                    for button in buttons: # Check for collision with EXISTING buttons
                        if button.rect.collidepoint(cur_x,cur_y):
                               #########
                               if "Select Existing" in button.text:
                                    button.UP_DN = "DN"
                                    print("existing selected")
                                    cwd = os.getcwd()
                                    RODENT, screw_holes_scrn, screw_holes_rat, craniotomies_scrn, craniotomies_rat = CranioHelperFunctions.load_design(cwd,graphic_scaleXY ,BREGMAscrn)
                                    print("screw_holes_scrn: ",screw_holes_scrn)
                                    print("craniotomies_rat: ",craniotomies_rat)
                                    if "RAT" in RODENT:
                                        RATS = True
                                        MICE = False
                                    if "M" in RODENT:
                                        RATS = False
                                        MICE = True
                               elif "Create" in button.text:
                                    print("Create New Craniotomy")
                                    button.UP_DN = "DN"
                                    subprocess.call("CraniotomyDesigner1.3.py", shell=True)

                               # DRILL Y
                               elif button.index == 2 or button.index == 12 or button.index == 22: # y"^" Move drill to UP (+y)
                               ...
                               # SELECT BEST-FIT PLANE POINTS
                               elif "SELECT POINT" in button.text or "SELECT BREGMA" in button.text:
                                    button.UP_DN = "DN"
                                    #n = len(roboXs)+1
                                    ...
                                    if n == 1: # Bregma in screen coord
                                        ...
                                        info_boxes[1].text = str(BREGMArat[0]) +', '+ \
                                                             str(BREGMArat[1]) +', '+ \
                                                             str(BREGMArat[2]) 
                                    ...
                                    else: # For screen graphics when ROBOT is NOT connected
...
##                               ##########################################
##
##                               elif button.index == 40: # Circular Craniotomy
##                                     CIRC_CRANIOTOMY = True
##                                    
##                               elif button.index == 41: # Draw Craniotomy Path
##                                     CIRC_CRANIOTOMY = False
##                               ############################################
                               elif button.index == 9: # "Craniotomy":
                                    button.UP_DN = "DN"
                                    cranios_combined =[]
                                    c3d = []
                                    print("craniotomies: ",craniotomies_rat)
                                    print("BREGMArobo :",BREGMArobo)
                                    # Turn 2D craniotomies into 3D at BREGMA depth
                                    for c in craniotomies_rat:  # NOTE:  At this point, crniotomies are only list of (x,Y), no z
                                        for p in c:
                                            c3d.append((p[0]+BREGMArobo[0],p[1]+BREGMArobo[1],BREGMArobo[2]))

                                        c3d.append((p[0]+BREGMArobo[0],p[1]+BREGMArobo[1],BREGMArobo[2]-10)) # create new point 10 mm above last point
                                        print ("c3d ",c3d)
                                        
##                                    for c in c3d:
##                                        cranios_combined.append(c)


                                   # print("cranios_combined: ",c3d)
                                    
                                    # XXXX RUN CRANIOTOMIES THROUGH BEST FIT PLANE FIRST XXXXXX
                                    if ROBOT_CONNECTED:
                                        #for cranio in craniotomies:
                                        cranThread = threading.Thread(target = drill_carniotomy,
                                                                      args = (BREGMArobo,Mx,My,Mz, c3d, depth, queCran))
                                        cranThread.start()
...
##                                    if ROBOT_CONNECTED:
##                                        print("CRANIOTOMY STARTED....")
##                                        #for cranio in craniotomies:
##                                    if circle is not None: # This is the projected circle onto the best fit plabe
##
##
##                                        circle = best_fit_plot(roboXs,roboYs,roboZs,5.5,BREGMArobo,0,-1.7)
##                                        cranThread = threading.Thread(target = drill_circ_carniotomy,
...
##                                            move_to_hole.start()
                                        ...
                                        print("Bregma MUST be selected first!!!")
                               ############################################         
                               ...
                               ############################################# 
                                    ...
                                    queCran.put('vel_dn')
                                    drill_vel -= 0.25
                                    if drill_vel <= 0.25: drill_vel = 0.25# This is for diplay only!
                                                      # Not directly tied to drill vel in craniotomy
        ...
        if RATS:  myscreen.blit(rats_skull_graphic,(94,350))
        if MICE:  myscreen.blit(mice_skull_graphic,(124,361))
        #########################
        #  Create GUI elements
        #########################
        for button in buttons:
        ...
        for user_input in user_inputs:
            if CIRC_CRANIOTOMY:
                ...
                if not "5.)"  in user_input.label and not "6.)" in user_input.label and  not "7.)" in user_input.label: 
                    user_input.draw()
        ...
        if not BREGMA_SELECTED:
            drill.draw_drillZ(660, 505)
            
        # DRAW DRILL ACTUAL LOCATION
        else: # BREGMA_SELECTED:
            drill.draw_drillXY(int(DRILLscrn[0]),int(DRILLscrn[1]))
            drill.draw_drillZ(660, int(DRILLscrn[2]))

            if len(craniotomies_scrn)  > 0:
                 #print("len(craniotomies_scrn: ",len(craniotomies_scrn))
                 for c in craniotomies_scrn:
                         p = 0
                         for point in c:
                                 hcolor = (0,0,255)
                                 cranotomies_robo = (int(point[0]+ BREGMArobo[0]),
                                                 int(point[1] + BREGMArobo[1]))# in Robot coords
                                
        ##                         craniotomies_scrn = ((int(point[0] - BREGMAscrn[0])/graphic_scaleXY),
        ##                                         (int(point[1]-BREGMAscrn[1])/graphic_scaleXY))

                                 pygame.draw.circle(myscreen, (0,0,255), point, 3,2) # BLUE
                                 if p > 0:
                                        # Draw line between points
                                        pygame.draw.line(myscreen,hcolor,(prev_point),(point),1)

                                 prev_point = point
                                 p+=1

                         # Draw line between ist and last point
                         pygame.draw.line(myscreen,hcolor,(c[0]),(c[len(c)-1]),1)
                                    
            if len(screw_holes_scrn) > 0:
                for hole in screw_holes_scrn:
                    hcolor = (150,255,255)
                    location_robo = (int(hole[0]+ BREGMArobo[0]),
                                     int(hole[1] + BREGMArobo[1]))# in Robot coords
                    
                    location_scrn = ((int(hole[0] - BREGMAscrn[0])/graphic_scaleXY),
                                     (int(hole[1]-BREGMAscrn[1])/graphic_scaleXY))

                    #draw_screw_hole(myscreen,location_scrn, hcolor)
                    pygame.draw.circle(myscreen, (0,0,255), hole, 3,2) # BLUE
                    pygame.draw.line(myscreen,(0,0,255),(hole[0],hole[1] - 10),(hole[0], hole[1] + 10),1)
                    pygame.draw.line(myscreen,(0,0,255),(hole[0] -10 ,hole[1]),(hole[0]+10, hole[1]),1)
